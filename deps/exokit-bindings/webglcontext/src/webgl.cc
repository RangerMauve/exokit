...
static const float PORTAL_POSITIONS[] = {
  0.5f, 0.5f, 0.0f, 0.5f, 0.5f, -1.0f, 0.5f, -0.5f, 0.0f, 0.5f, -0.5f, 0.0f, 0.5f, 0.5f, -1.0f, 0.5f, -0.5f, -1.0f, -0.5f, 0.5f, -1.0f, -0.5f, 0.5f, 0.0f, -0.5f, -0.5f, -1.0f, -0.5f, -0.5f, -1.0f, -0.5f, 0.5f, 0.0f, -0.5f, -0.5f, 0.0f, -0.5f, 0.5f, -1.0f, 0.5f, 0.5f, -1.0f, -0.5f, 0.5f, 0.0f, -0.5f, 0.5f, 0.0f, 0.5f, 0.5f, -1.0f, 0.5f, 0.5f, 0.0f, -0.5f, -0.5f, 0.0f, 0.5f, -0.5f, 0.0f, -0.5f, -0.5f, -1.0f, -0.5f, -0.5f, -1.0f, 0.5f, -0.5f, 0.0f, 0.5f, -0.5f, -1.0f, 0.5f, 0.5f, -1.0f, -0.5f, 0.5f, -1.0f, 0.5f, -0.5f, -1.0f, 0.5f, -0.5f, -1.0f, -0.5f, 0.5f, -1.0f, -0.5f, -0.5f, -1.0f,
};
...
StencilGlShader::StencilGlShader() {
  ...
  // vertex array
  glBindVertexArray(this->stencilVao);

  // vertex shader
  GLuint stencilVertex = glCreateShader(GL_VERTEX_SHADER);
  glShaderSource(stencilVertex, 1, &stencilVsh, NULL);
  glCompileShader(stencilVertex);
  GLint success;
  glGetShaderiv(stencilVertex, GL_COMPILE_STATUS, &success);
  if (!success) {
    char infoLog[4096];
    GLsizei length;
    glGetShaderInfoLog(stencilVertex, sizeof(infoLog), &length, infoLog);
    infoLog[length] = '\0';
    exout << "stencil vertex shader compilation failed:\n" << infoLog << std::endl;
    return;
  };

  // fragment shader
  GLuint stencilFragment = glCreateShader(GL_FRAGMENT_SHADER);
  glShaderSource(stencilFragment, 1, &stencilFsh, NULL);
  glCompileShader(stencilFragment);
  glGetShaderiv(stencilFragment, GL_COMPILE_STATUS, &success);
  if (!success) {
    char infoLog[4096];
    GLsizei length;
    glGetShaderInfoLog(stencilFragment, sizeof(infoLog), &length, infoLog);
    infoLog[length] = '\0';
  ...
  glAttachShader(this->stencilProgram, stencilVertex);
  glAttachShader(this->stencilProgram, stencilFragment);
  glLinkProgram(this->stencilProgram);
  glGetProgramiv(this->stencilProgram, GL_LINK_STATUS, &success);
  if (!success) {
    char infoLog[4096];
    GLsizei length;
  ...
  this->positionLocation = glGetAttribLocation(this->stencilProgram, "position");
...
StencilGlShader::~StencilGlShader() {}
GlKey StencilGlShader::key = GlKey::GL_KEY_STENCIL;
